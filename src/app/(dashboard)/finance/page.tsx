...
export default function FinancePage() {
  ...
  const totalAssets = accounts
    .filter(acc => acc.account_type === 'cash' || acc.account_type === 'current' || acc.account_type === 'savings')
  ...
  const handleSaveAccount = async () => {
    ...
    try {
      ...
      toast.success(
        selectedAccount ? "Account updated!" : "Account created!"
      );
  ...
  const handleSavePayment = async () => {
    if (!selectedPayable || !paymentFormData.bank_account_id || paymentFormData.amount <= 0) {
      return toast.error("Please select a bank account and enter a valid amount.");
    }
  ...
  const paymentAccounts = accounts.filter(acc => acc.account_type === 'cash' || acc.account_type === 'current');

  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-bold tracking-tight">Finance</h1>
      <p className="text-muted-foreground">
        Manage your bank accounts, loans, and supplier payments (Accounts Payable).
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">Total Cash (Assets)</CardTitle>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">Total Loans (Liabilities)</CardTitle>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList>
          ...
          <TabsTrigger value="payables">Accounts Payable (Supplier Dues)</TabsTrigger>
        </TabsList>
            ...
            <CardContent>
              {loading ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {accounts.map((acc) => {
                      const isLiability = acc.account_type === 'loan' || acc.account_type === 'od';
                      return (
                        <TableRow key={acc.id}>
                          <TableCell className="font-medium">{acc.account_name}</TableCell>
                          <TableCell>{acc.account_type === 'cash' ? 'Cash in Hand' : acc.banks.bank_name}</TableCell>
                          <TableCell>{acc.account_number}</TableCell>
                          <TableCell className="capitalize">{acc.account_type}</TableCell>
                          <TableCell className={`text-right font-bold ${isLiability ? 'text-destructive' : 'text-green-600'}`}>
                            {formatCurrency(acc.current_balance)}
                          </TableCell>
                          <TableCell className="text-right">
                            <Button variant="ghost" size="icon-sm" onClick={() => openAccountDialog(acc)}>
                ...
                <Table>
                  ...
                  <TableBody>
                    {payables.length === 0 ? (
                      ...
                      payables.map((p) => (
                        <TableRow key={p.id}>
                          <TableCell className="font-medium">{p.purchase_id}</TableCell>
                          <TableCell>{new Date(p.purchase_date).toLocaleDateString()}</TableCell>
                          <TableCell>{p.suppliers.name}</TableCell>
                          <TableCell>
                            <Badge variant={p.payment_status === 'unpaid' ? 'destructive' : 'outline'} className="capitalize">
                              {p.payment_status}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-right">{formatCurrency(p.total_amount)}</TableCell>
                          <TableCell className="text-right font-bold text-destructive">{formatCurrency(p.balance_due)}</TableCell>
                          <TableCell className="text-right">
                            <Button size="sm" onClick={() => openPaymentDialog(p)}>
          ...
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(val: BankAccount["account_type"]) => setAccountFormData({ ...accountFormData, account_type: val, bank_id: val === 'cash' ? banks.find(b => b.bank_code === 'CASH')?.id || '' : '' })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            {accountFormData.account_type !== 'cash' && (
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(val) => setAccountFormData({ ...accountFormData, bank_id: val })}
                >
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {banks.map(bank => (
                      <SelectItem key={bank.id} value={bank.id}>{bank.bank_name} ({bank.bank_code})</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              </div>
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder={accountFormData.account_type === 'cash' ? 'CASH-001' : '123456789'}
              />
            </div>
            <div className="space-y-2">
              ...
              />
               <p className="text-xs text-muted-foreground">
                For Loans or Overdrafts, enter this as a negative number (e.g., -50000).
              </p>
            </div>
          </div>
          <DialogFooter>
      ...
      <Dialog open={isPaymentDialogOpen} onOpenChange={setIsPaymentDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Pay supplier: <strong>{selectedPayable?.suppliers.name}</strong><br/>
              For Purchase Order: <strong>{selectedPayable?.purchase_id}</strong><br/>
              Amount Due: <strong>{formatCurrency(selectedPayable?.balance_due || 0)}</strong>
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              <Label htmlFor="pay-method">Payment Method</Label>
               <Select
                ...
                onValueChange={(val: any) => setPaymentFormData({ ...paymentFormData, payment_method: val })}
              >
                <SelectTrigger>
                  <SelectValue />
                </SelectTrigger>
            ...
            <div className="space-y-2">
              <Label htmlFor="pay-account">Pay From Account</Label>
               <Select
                ...
                onValueChange={(val) => setPaymentFormData({ ...paymentFormData, bank_account_id: val })}
              >
                <SelectTrigger>
                ...
                <SelectContent>
                  {paymentAccounts.map(acc => (
                    <SelectItem key={acc.id} value={acc.id}>
                      {acc.account_name} ({acc.account_type}) - {formatCurrency(acc.current_balance)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
